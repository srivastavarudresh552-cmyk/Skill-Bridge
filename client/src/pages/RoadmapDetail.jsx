  ...
  return (
    <div className="flex items-start gap-3 rounded-lg px-2 py-2.5 transition-colors hover:bg-gray-50">
      ...
      <div className="min-w-0 flex-1">
        <p
          className={`text-sm font-medium transition-colors ${step.completed ? "text-gray-400 line-through" : "text-gray-800"}`}
        >
          {step.title}
        </p>
        <p
          className={`mt-0.5 text-sm ${step.completed ? "text-gray-300" : "text-gray-500"}`}
        >
          {step.description}
        </p>
...
export default function RoadmapDetail() {
  ...
  const sortedSteps = [...roadmap.roadmapSteps].sort(
    (a, b) => a.order - b.order,
  );
        ...
        <div className="flex flex-wrap gap-2">
          {roadmap.matchedSkills.length > 0 ? (
            ...
            <p className="text-sm text-gray-400">
              No strong matches found — that's okay, your roadmap covers what to
              build.
            </p>
          )}
        </div>
      ...
      <div className="card animate-fade-in-up mb-6 p-6">
        ...
        <div className="space-y-3">
          {roadmap.gapSkills.map((gap, i) => (
            ...
            >
              <Badge tone={priorityTone[gap.priority] || "low"}>
                {gap.priority}
              </Badge>
